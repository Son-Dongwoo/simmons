...
class SerialModule : public rclcpp::Node {
  ...
  private:
    ...
    void uart_tx() {
      ...
      //float person_length = 12.3;
      //float person_degree = 78.9;
      ...
      if (3 <= mode_ && mode_ <= 6)
      {
        ...
        // RCLCPP_INFO(this->get_logger(), "TX) Person Length: %.3f, Person Degree: %.3f, Mode: %d", person_length_, person_degree_, mode_);
      } 
    ...
    void uart_rx() {
      ...
      {
      	//RCLCPP_INFO(this->get_logger(), "RX) FX: %.3f, FY: %.3f, FZ: %.3f", FX-20000.0, FY-20000.0, FZ-20000.0);
      }
