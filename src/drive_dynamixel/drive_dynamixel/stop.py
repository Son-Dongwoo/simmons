...
class DynamixelController(Node):
    def __init__(self):
        ...
        self.set_multi_goal_position([3072, 3072, 1024, 1024])
    ...
    def steering_kinematics(self, dist_cam, angle_cam):   ### camera data to steer angle calculation   
        ...
        dx = (L_shift) + dist_cam*math.cos(deg2rad(angle_cam))
        dy = dist_cam*math.sin(deg2rad(angle_cam))
        
        ### center to object distance and angle
        dist_cen = math.sqrt(dx**2 + dy**2)
        angle_cen = rad2deg(math.atan2(dy, dx))
        
        if (angle_cen == 0.0):
            # rot_radius = dist_cen/(0.000000000000001)
            rot_radius = dist_cen/(1e-15) 
        else:
            rot_radius = dist_cen/(2.0*math.sin(deg2rad(angle_cen)))
